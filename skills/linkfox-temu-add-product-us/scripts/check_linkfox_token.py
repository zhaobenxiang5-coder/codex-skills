...
def main():
    ...
    auth_code = result.get("errcode", result.get("code"))
    if (
        is_linkfox_auth_error(result)
        or auth_code in {401, 403, "401", "403"}
        or "authorized error" in auth_text
        or "unauthorized" in auth_text
    ):
        print(
            json.dumps(
                {
                    ...
                    "message": result.get("message") or result.get("errmsg") or result,
    ...
    if str(result.get("error") or "").startswith("Connection failed"):
        print(
            json.dumps(
                {
                    "ok": False,
                    "reason": "linkfox_gateway_unreachable",
